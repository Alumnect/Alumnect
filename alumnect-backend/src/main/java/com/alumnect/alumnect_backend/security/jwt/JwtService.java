 ...
 * Hỗ trợ tạo mới token, giải mã token để lấy thông tin (claims) và kiểm tra tính hợp lệ của token.
 */
@Service
public class JwtService {
    
    // Khóa bí mật dùng để ký token mã hóa Base64 (được tiêm vào từ file cấu hình/môi trường)
    @Value("${app.jwt.secret}")
    private String keySecret;

    // Thời gian sống của Token (mili-giây), được tiêm vào từ file cấu hình/môi trường
    @Value("${app.jwt.expiration}")
    private long jwtExpiration;

    /**
     * Tạo token JWT mới dựa trên thông tin người dùng.
     * Lưu trữ các thông tin bổ sung (claims): ID, email, vai trò (role) và trạng thái duyệt hồ sơ.
     *
    ...
    /**
     * Xác thực token: email trong token trùng với email UserDetails và token chưa hết hạn.
     *
