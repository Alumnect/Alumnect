...
-- sắp xếp đa tiêu chí - multi-sort). Gắn thẳng vào chủ đề con (đã seed ở V16).
--
-- Bộ dữ liệu cố ý có:
...
--   * topic_id lấy theo tên chủ đề (đã seed ở V5 + V16); không khớp thì NULL.
-- =====================================================================

INSERT INTO questions (author_id, topic_id, title, body, status, vote_count, answer_count)
SELECT
    (SELECT id FROM users
        WHERE role_id IN (SELECT id FROM roles WHERE name IN ('STUDENT', 'ALUMNI'))
        ORDER BY id LIMIT 1),
    (SELECT id FROM forum_topics WHERE name = v.topic_name),
...
FROM (VALUES
    ('Roadmap học Software Engineering cho người mới ra trường?',
     ...
     30, 15, 'Backend (Java/Node/Python)'),
    ...
    ('Cách chuẩn bị hồ sơ xin học bổng thạc sĩ ngành CNTT?',
     ...
     18, 9, 'Education')
) AS v(title, body, votes, answers, topic_name)
