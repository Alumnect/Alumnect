...
public class AuthServiceImpl implements AuthService {
     ...
     * Bao gồm mã hóa mật khẩu, tạo thông tin cá nhân (Profile), tạo cài đặt mặc định (Settings),
     * nếu là ALUMNI thì tạo phiếu yêu cầu xác minh tư cách cựu sinh viên gửi lên Admin, và cuối cùng gửi email mã OTP xác nhận.
     */
    @Override
    @Transactional
    public void register(RegisterRequest request) {
        ...
        if (existingUserOpt.isPresent()) {
            ...
            if (user.getAccountStatus() == AccountStatus.WAITING_APPROVAL) {
                throw new ConflictException("Tài khoản của bạn đã xác thực email thành công và đang chờ quản trị viên phê duyệt. Vui lòng đợi admin duyệt.");
                ...
                throw new ConflictException("Tài khoản liên kết với email này đã bị khóa. Vui lòng liên hệ quản trị viên.");
        ...
        // 6. Tạo thực thể cài đặt mặc định UserSettings (Theme: SYSTEM, Ngôn ngữ: vi) nếu là user mới
        if (isNewUser) {
        ...
        // 7. Nếu vai trò đăng ký là ALUMNI, tạo/cập nhật phiếu yêu cầu xác minh tư cách cựu sinh viên
        ...
        // 10. Gửi email chứa mã OTP và link xác nhận trực tiếp tới địa chỉ email người dùng
     ...
     * - ALUMNI → đưa về trạng thái WAITING_APPROVAL (chờ Admin xét duyệt phiếu minh chứng)
     */
    /**
    ...
    public String verifyEmail(String email, String tokenString) {
        ...
        VerificationToken token = tokenRepository.findFirstByUserAndTypeOrderByCreatedAtDesc(user, VerificationType.EMAIL_VERIFICATION)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy yêu cầu xác thực email nào cho tài khoản này"));
    ...
    public void resendOtp(String email) {
        ...
        // 1. Giới hạn thời gian chờ gửi lại mã mới (5 phút - bỏ qua nếu mã cũ đã bị khóa)
        Optional<VerificationToken> lastTokenOpt = tokenRepository.findFirstByUserAndTypeOrderByCreatedAtDesc(user, VerificationType.EMAIL_VERIFICATION);
