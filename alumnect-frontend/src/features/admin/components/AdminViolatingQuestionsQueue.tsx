...
import {
  ...
  Trash2, Info, UserX, HelpCircle, Radio
} from 'lucide-react'
import { PageHeader, Badge, Card, Avatar, EmptyState, Skeleton, toast } from '@/components/ui'
import { Button } from '@/components/ui/Button'
import { Reveal } from '@/components/motion'
import { cn } from '@/lib/utils'
import { useAdminQuestionReports, useUpdateQuestionReportStatus } from '../hooks/useAdminQuestionReports'
import { useWebSocketViolatingQuestions } from '../hooks/useWebSocketViolatingQuestions'
 ...
 */
export function AdminViolatingQuestionsQueue() {
  ...
  const { isConnected: isWsConnected } = useWebSocketViolatingQuestions()
  ...
  return (
    <div className="mx-auto max-w-6xl">
      {/* Tiêu đề trang & Huy hiệu kết nối Real-time */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        ...
        />
        <div className="flex items-center gap-2 self-start sm:self-center bg-white px-3.5 py-1.5 rounded-full border border-plum-900/10 shadow-2xs">
          <Radio size={14} className={cn('animate-pulse', isWsConnected ? 'text-emerald-500' : 'text-plum-300')} />
          <span className="text-xs font-bold text-plum-800">
            {isWsConnected ? 'Real-time Live' : 'Đang kết nối WebSocket...'}
          </span>
        </div>
      </div>
