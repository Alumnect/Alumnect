...
public class MailServiceImpl implements MailService {
    ...
    /**
     * Thực hiện tạo khuôn mẫu HTML và gửi email chứa mã OTP khôi phục mật khẩu.
     * Nếu không có kết nối SMTP hoặc lỗi xảy ra, thông tin sẽ được in ra console để
     * phát triển cục bộ dễ dàng.
     */
    @Override
    ...
    public void sendPasswordResetEmail(String toEmail, String token, String fullName) {
        String subject = "Đặt lại mật khẩu tài khoản AlumNect";
        String htmlContent = "<div style=\"font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f3f4f6; padding: 40px 10px; margin: 0; min-height: 100%;\">"
                + "  <div style=\"max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06);\">"
                + "    <!-- Top Banner with FPT/AlumNect color gradient -->"
                + "    <div style=\"background: linear-gradient(135deg, #1e3a8a 0%, #f97316 100%); padding: 30px; text-align: center;\">"
                ...
                + "    <div style=\"padding: 40px 30px; color: #334155; line-height: 1.6;\">"
                + "      <h3 style=\"color: #1e3a8a; margin-top: 0; font-size: 20px;\">Xin chào " + fullName + ",</h3>"
                + "      <p style=\"font-size: 16px;\">Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản AlumNect của bạn.</p>"
                + "      <p style=\"font-size: 16px;\">Dưới đây là mã OTP khôi phục mật khẩu. Vui lòng nhập mã này vào trang khôi phục để thiết lập mật khẩu mới:</p>"
                + "      "
                + "      <!-- OTP Display Box -->"
                + "      <div style=\"text-align: center; margin: 35px 0;\">"
                + "        <div style=\"display: inline-block; background-color: #f8fafc; border: 2px dashed #cbd5e1; border-radius: 10px; padding: 15px 35px; font-size: 32px; font-weight: 800; letter-spacing: 6px; color: #f97316; font-family: 'Courier New', Courier, monospace;\">"
                ...
                + "      "
                + "      <p style=\"font-size: 14px; color: #64748b; margin-top: 30px; border-left: 3px solid #f97316; padding-left: 10px;\">"
                + "        <strong>Lưu ý:</strong> Mã xác thực này có hiệu lực trong vòng <strong>5 phút</strong> và chỉ được sử dụng một lần duy nhất. Nếu bạn không gửi yêu cầu này, vui lòng bỏ qua email và bảo mật tài khoản của mình."
                + "      </p>"
                + "    </div>"
                + "    "
                + "    <!-- Footer -->"
        ...
        try {
            ...
            mailSender.send(message);
            log.info("Gửi email đặt lại mật khẩu thành công tới: {}", toEmail);
        } catch (Exception e) {
            log.error("Gửi email đặt lại mật khẩu qua SMTP thất bại: {}. HIỂN THỊ TRÊN CONSOLE.", e.getMessage());
            System.out.println("====================================================================");
            System.out.println("ALUMNECT - DỊCH VỤ EMAIL GIẢ LẬP ĐẶT LẠI MẬT KHẨU");
            System.out.println("Tới: " + toEmail);
            System.out.println("Tiêu đề: " + subject);
            System.out.println("Mã OTP Đặt lại mật khẩu: " + token);
            System.out.println("====================================================================");
        }
    }
}
